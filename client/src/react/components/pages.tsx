...
import {
    AuthFlowType,
    B2BProducts,
    type Member, type StytchAPIError,
...
import {StytchB2B, useStytchB2BClient, useStytchMember, useStytchMemberSession} from "@stytch/react/b2b";
...
export const Account = () => {
    ...
    const onUpdateName = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        const newName = new FormData(e.currentTarget).get("name") as string;

        stytch.self.update({name: newName});
    }
    return (
...
export const Login = () => {
    ...
    const [showEMLOTP, setShowEMLOTP] = useState(false);
    const [otpError, setError] = useState('');
    const stytch = useStytchB2BClient();
    const hasCalledLoginOrSignup = useRef(false);
    const navigate = useNavigate();

    useEffect(() => {
        if (!session || !member) return;

        console.dir(session.authentication_factors)

        // TODO: SSO factor here
        const hasSMSOTP = session.authentication_factors.find(factor => factor.type === "TODO");
        const hasTOTP = session.authentication_factors.find(factor => factor.type === "TODO");
        const hasEmailOTP = session.authentication_factors.find(factor => factor.type === "email_otp");

        const hasSecondFactor = hasSMSOTP || hasTOTP || hasEmailOTP;
        ...
        if (hasCalledLoginOrSignup.current) return;
        if (!hasSecondFactor) {
            hasCalledLoginOrSignup.current = true;
            setShowEMLOTP(true);
    ...
    }, [session, member, showEMLOTP, stytch, navigate]);
    ...
    return (
        <>
            {!showEMLOTP && <StytchB2B config={discoveryConfig} callbacks={{
                onEvent(event: StytchEvent<Stytch.DefaultProjectConfiguration>) {
                    if (event.type === StytchEventType.AuthenticateFlowComplete) {
                        console.log("wheeee!")
                    }
                }
            }}/>}
            {showEMLOTP && <div className={styles.loginForm}>
              <p>Email OTP step</p>
              <form onSubmit={onOTPSubmit}>
                <label htmlFor="otp">Enter the code from your email</label>
                <input type="text" name="otp" id="otp" className={styles.emailInput} required/>
